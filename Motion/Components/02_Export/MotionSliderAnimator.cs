    ...
    public class MotionSliderAnimator : GH_SliderAnimator, IDisposable
    {
            ...
            Action<int, int> progressCallback = null)
        {
            ...
            try
            {
                ...
                if (cycles != null && isCycles && isRaytracedMode)
                {
                ...
                while (m_currentValue <= maxValue)
                {
                    ...
                    // 创建并保存帧
                    using (bitmap = MotionCreateFrame(isTransparent, myView, isCycles, isRaytracedMode, realtimeRenderPasses))
                    {
                        if (bitmap == null)
                        {
        ...
        private void UpdateCommandPrompt(long startTicks, double currentValue, double maxValue, double minValue)
        {
            if (UseCustomRange)
            {
                RhinoApp.CommandPrompt =
                    $"Generating frame {m_frameIndex + (int)CustomRange.Min} of {(int)CustomRange.Min}-{(int)CustomRange.Max}. | Total:{(int)CustomRange.Max - (int)CustomRange.Min + 1} | Time left: {EstimateTimeLeftFormatted(startTicks)}";
            }
            else
            {
                RhinoApp.CommandPrompt =
                    $"Generating frame {m_frameIndex} of {m_owner.Slider.Minimum}-{m_owner.Slider.Maximum}. | Total:{m_owner.Slider.Maximum - m_owner.Slider.Minimum + 1} | Time left: {EstimateTimeLeftFormatted(startTicks)}";
            }
        }
        ...
        public Bitmap MotionCreateFrame(bool isTransparent, RhinoView myView, bool isCycles,
            bool isRaytracedMode, int realtimeRenderPasses)
        {
            if (myView == null)
                return null;
