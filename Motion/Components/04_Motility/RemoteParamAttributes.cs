    ...
    public class RemoteParamAttributes : GH_FloatingParamAttributes
    {
        ...
        protected override void Render(GH_Canvas canvas, Graphics graphics, GH_CanvasChannel channel)
        {
            ...
            if (channel == GH_CanvasChannel.Objects)
            {
                ...
                if (isVisible)
                {
                    ...
                    {
                        var remoteParam = receiver as RemoteParam;
                        if (remoteParam != null)
                        {
                            string status = "";
                            ...
                            else if (remoteParam._hideWhenEmpty)
                            {
                                status = "HIDE";
                                textColor = Color.DodgerBlue;
                            }
                            else if (remoteParam._lockWhenEmpty)
                            {
                                status = "LOCK";
                            ...
                            if (!string.IsNullOrEmpty(status))
                            {
                                ...
                                {
                                    graphics.DrawString(status, font, brush, textRect, format);
                                }
                            }
                        }
                    }
        ...
        public override GH_ObjectResponse RespondToMouseDoubleClick(GH_Canvas sender, GH_CanvasMouseEvent e)
        {
            ...
            if (Owner is Param_RemoteSender)
            {
                ...
                newReceiver.NickName = Owner.NickName;
            ...
            {
                // 创建 Data Param
                var newDataParam = new Param_RemoteData();
                ghDoc.AddObject(newDataParam, true, ghDoc.ObjectCount);
                
                // 链接到 Receiver
                newDataParam.LinkToReceiver(receiver);
                
                // 放置在 Receiver 的左侧
                newDataParam.Attributes.Pivot = new PointF(Pivot.X - 50, Pivot.Y);
